...
export default function LangToggle({ className = "" }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-1 bg-white/70 backdrop-blur ${className}`}
      ...
      {LANGS.map((l) => (
        <button
          ...
          className={`text-[11px] font-semibold rounded-full px-2 py-0.5 transition-colors ${
            lang === l.key ? "bg-brand-600 text-white" : "hover:bg-brand-50"
          }`}
