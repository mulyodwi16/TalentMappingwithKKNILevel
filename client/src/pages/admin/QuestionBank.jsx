...
export default function QuestionBank() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(t("Soal disimpan")); qc.invalidateQueries(["admin-questions"]); setModal(null); },
    onError: (err) => toast.error(err || t("Gagal")),
  });

  const del = useMutation({
    mutationFn: (id) => api.delete(`/admin/questions/${id}`),
    onSuccess: () => { toast.success(t("Soal dihapus")); qc.invalidateQueries(["admin-questions"]); },
    onError: (err) => toast.error(err || t("Gagal")),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h2 className="text-xl font-bold text-white">{t("Bank Soal")}</h2>
        <div className="flex gap-2 flex-wrap">
          <select className="input text-sm py-1.5 w-auto" value={filter.level}
            onChange={(e) => setFilter((f) => ({ ...f, level: e.target.value }))}>
            <option value="">{t("Semua Level")}</option>
          ...
          <select className="input text-sm py-1.5 w-auto" value={filter.competency}
            ...
            <option value="">{t("Semua Kompetensi")}</option>
            {competencies.map((c) => <option key={c.code} value={c.code}>{c.name}</option>)}
          </select>
          <button onClick={openCreate} className="btn-primary text-sm py-1.5 px-3">{t("+ Tambah Soal")}</button>
        </div>
      </div>
          ...
          <div key={q.id} className="card p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex gap-1.5 flex-shrink-0">
                <button onClick={() => openEdit(q)} className="text-xs px-2.5 py-1 rounded-lg bg-slate-700 hover:bg-slate-600 text-white">{t("Edit")}</button>
                <button onClick={() => { if (confirm(t("Hapus soal ini?"))) del.mutate(q.id); }}
                  className="text-xs px-2.5 py-1 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400">{t("Hapus")}</button>
              </div>
            </div>
          </div>
        ))}
        {questions.length === 0 && (
          <div className="card p-12 text-center text-slate-500">{t("Belum ada soal. Klik \"+ Tambah Soal\".")}</div>
        )}
      </div>
          ...
          <div className="card p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); save.mutate({ ...form, answerKey: parseInt(form.answerKey), kkniLevel: parseInt(form.kkniLevel), points: parseInt(form.points) }); }} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-1.5 block">{t("Kompetensi")}</label>
                ...
                <div>
                  ...
                  <select className="input" value={form.kkniLevel} onChange={(e) => setForm((f) => ({ ...f, kkniLevel: e.target.value }))}>
                    {[3,4,5,6,7,8,9].map((l) => <option key={l} value={l}>{rankName(l)} ({t("setara level KKNI {n}", { n: l })})</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-300 mb-1.5 block">{t("Pertanyaan")}</label>
                <textarea className="input h-20 resize-none" value={form.question} onChange={(e) => setForm((f) => ({ ...f, question: e.target.value }))} required />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-300 mb-2 block">{t("Pilihan Jawaban")}</label>
                {form.options.map((opt, i) => (
                  <div key={i} className="flex items-center gap-2 mb-2">
                    ...
                    <input className="input text-sm py-1.5 flex-1" value={opt}
                      ...
                      placeholder={t("Pilihan {x}", { x: String.fromCharCode(65+i) })} required />
                    {form.answerKey === i && <span className="text-emerald-400 text-xs font-medium">{t("✓ Benar")}</span>}
                  </div>
                ))}
              </div>
