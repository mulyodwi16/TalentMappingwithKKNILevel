...
export default function RuleManagement() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(t("Aturan disimpan")); qc.invalidateQueries(["admin-rules"]); setModal(null); },
    onError: (err) => toast.error(err || t("Gagal")),
  });

  const del = useMutation({
    mutationFn: (id) => api.delete(`/admin/rules/${id}`),
    onSuccess: () => { toast.success(t("Aturan dihapus")); qc.invalidateQueries(["admin-rules"]); },
    onError: (err) => toast.error(err || t("Gagal")),
  });
              ...
              <tr key={r.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1.5">
                    <button onClick={() => openEdit(r)} className="text-xs px-2.5 py-1 rounded-lg bg-slate-700 hover:bg-slate-600 text-white">{t("Edit")}</button>
                    <button onClick={() => { if (confirm(t("Hapus aturan ini?"))) del.mutate(r.id); }}
        ...
        <div className="is-modal fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="card p-6 w-full max-w-sm">
            ...
            <form onSubmit={(e) => { e.preventDefault(); save.mutate({ ...form, conditions: { ...form.conditions, minExperience: form.conditions.minExperience ? parseInt(form.conditions.minExperience) : undefined } }); }} className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-slate-300 mb-1.5 block">{t("Kata kunci sertifikasi (opsional)")}</label>
                <input className="input" type="text" placeholder={t("mis: editing, premiere")} value={form.conditions.cert} onChange={(e) => upd("conditions.cert", e.target.value)} />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-300 mb-1.5 block">{t("Min. pengalaman (tahun, opsional)")}</label>
