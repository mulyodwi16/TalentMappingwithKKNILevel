  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Kontrol penuh atas seluruh sistem</p>
      </div>
      ...
      {stats?.recentLogs?.length > 0 && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/app/admin/audit" className="text-xs text-brand-400 hover:text-brand-300">Lihat semua →</Link>
          </div>
            ...
            {stats.recentLogs.map((log) => (
              <div key={log.id} className="flex items-center gap-3 py-2 border-b border-slate-800/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500">{log.actorEmail} · {new Date(log.createdAt).toLocaleString("id-ID")}</p>
                </div>
