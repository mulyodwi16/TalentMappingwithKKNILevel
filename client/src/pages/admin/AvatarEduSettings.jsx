...
export default function AvatarEduSettings() {
  ...
  async function save() {
    ...
    } catch (e) { toast.error(typeof e === "string" ? e : "Gagal simpan"); }
  ...
  async function testPreview() {
    ...
    catch (e) { toast.error(typeof e === "string" ? e : "Gagal memuat"); }
    finally { setLoadingPreview(false); }
  }

  if (!cfg) return <div className="text-center py-16" style={{ color: "var(--text-4)" }}>Memuat…</div>;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="card p-5 space-y-4">
        <label className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-base)" }}>Aktifkan integrasi AvatarEdu</p>
            <p className="text-xs" style={{ color: "var(--text-4)" }}>Jika nonaktif, course unggulan tidak ditampilkan.</p>
          </div>
        ...
        <div>
          <label className="text-sm font-semibold flex items-center gap-1.5" style={{ color: "var(--text-base)" }}><Search className="w-4 h-4" /> Kata kunci pencarian default</label>
          <p className="text-xs mb-1.5" style={{ color: "var(--text-4)" }}>Dipakai bila daftar slug unggulan kosong.</p>
          <input value={cfg.featuredQuery || ""} onChange={(e) => setCfg({ ...cfg, featuredQuery: e.target.value })} className="input text-sm" placeholder="kompetensi kerja, video editing…" />
        </div>

        <div>
          <label className="text-sm font-semibold flex items-center gap-1.5" style={{ color: "var(--text-base)" }}><Star className="w-4 h-4" /> Slug course unggulan (pisah koma)</label>
          <p className="text-xs mb-1.5" style={{ color: "var(--text-4)" }}>Bila diisi, hanya course ini yang jadi unggulan (menimpa kata kunci).</p>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={testPreview} disabled={loadingPreview} className="btn-outline text-sm py-2 px-4">{loadingPreview ? "Memuat…" : "Uji Pratinjau"}</button>
        </div>
      </div>

      {preview && (
        <div className="card p-5">
          <p className="text-sm font-semibold mb-3" style={{ color: "var(--text-base)" }}>Pratinjau course unggulan ({preview.length})</p>
          {preview.length === 0 ? <p className="text-sm" style={{ color: "var(--text-4)" }}>Tidak ada course — cek kata kunci/slug atau API key.</p> : (
