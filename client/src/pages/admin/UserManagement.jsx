...
export default function UserManagement() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(modal.mode === "create" ? "User dibuat" : "User diperbarui"); qc.invalidateQueries(["admin-users"]); setModal(null); },
    onError: (err) => toast.error(err || "Gagal"),
  });

  const del = useMutation({
    mutationFn: (id) => api.delete(`/admin/users/${id}`),
    onSuccess: () => { toast.success("User dihapus"); qc.invalidateQueries(["admin-users"]); },
    onError: (err) => toast.error(err || "Gagal"),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="btn-primary text-sm py-2 px-4">+ Tambah User</button>
      </div>
                ...
                <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      <button onClick={() => openEdit(u)} className="text-xs px-2.5 py-1 rounded-lg bg-slate-700 hover:bg-slate-600 text-white transition-colors">Edit</button>
                      <button onClick={() => { if (confirm(`Hapus ${u.name}?`)) del.mutate(u.id); }}
                        className="text-xs px-2.5 py-1 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 transition-colors">Hapus</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="card p-6 w-full max-w-md max-h-screen overflow-y-auto">
            <h3 className="font-semibold text-white mb-5">{modal.mode === "create" ? "Tambah User" : "Edit User"}</h3>
