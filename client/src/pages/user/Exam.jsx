...
import {
  CheckCircle2, Lock, BookOpen, PlayCircle, RotateCcw, GraduationCap, Award, ArrowLeft,
} from "lucide-react";
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="space-y-2">
        {units.map((u) => {
          ...
          return (
            <div key={u.code} className="card p-3.5 flex items-center gap-3">
              <div className={`w-8 h-8 rounded-full grid place-items-center shrink-0 text-xs font-black ${u.state === "passed" ? "bg-emerald-500 text-white" : "bg-[var(--bg-muted)]"}`} style={u.state === "passed" ? {} : { color: "var(--text-3)" }}>
                {u.state === "passed" ? "✓" : u.order}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium truncate" style={{ color: "var(--text-base)" }}>{u.title}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`text-[11px] px-1.5 py-0.5 rounded-full ${ui.badge}`}>{ui.label}</span>
                  {u.score != null && <span className="text-[11px]" style={{ color: "var(--text-4)" }}>skor {u.score}%</span>}
                  {u.questionCount ? <span className="text-[11px]" style={{ color: "var(--text-4)" }}>· {u.questionCount} soal</span> : null}
                </div>
              </div>
              {canExam ? (
                <button onClick={() => onPick(u.code)} className="btn-primary text-xs py-1.5 px-3 whitespace-nowrap flex items-center gap-1">
                  {u.state === "passed" ? <><RotateCcw className="w-3.5 h-3.5" /> Ulang</> : <><PlayCircle className="w-3.5 h-3.5" /> Ujian</>}
                </button>
              ) : (
                <Link to="/app/kelas" className="btn-outline text-xs py-1.5 px-3 whitespace-nowrap flex items-center gap-1">
                  <ui.Icon className="w-3.5 h-3.5" /> {u.state === "locked" ? "Buka" : "Belajar"}
                </Link>
              )}
            </div>
          );
...
export default function Exam() {
  ...
  const submit = useMutation({
    ...
    onSuccess: (data) => {
      setResult(data);
      ["attempts", "assessments", "profile", "notifications", "skkni-chosen", "kelas-units", "learning-path", "coins"].forEach((k) => qc.invalidateQueries([k]));
    },
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-4">
        <div className="card p-8 text-center">
          ...
          {isUnit ? (
            <p style={{ color: "var(--text-3)" }}>{result.passed ? "LULUS — sertifikat unit terbit ✓" : `Belum lulus (butuh ≥60%). ${result.correct}/${result.total} benar.`}</p>
          ) : (
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {q && (
        <div className="card p-6">
          <div className="text-xs text-brand-500 font-semibold mb-3">Unit: {q.competencyName || q.competencyCode}</div>
      ...
      <div className="flex flex-wrap gap-1.5 justify-center">
        {questions.map((q2, i) => (
          <button key={i} onClick={() => setCurrent(i)}
            className={`w-7 h-7 rounded-lg text-xs font-bold transition-all ${i === current ? "bg-brand-600 text-white" : answers[q2.id] !== undefined ? "bg-emerald-600/30 text-emerald-400" : ""}`}
            style={i === current || answers[q2.id] !== undefined ? {} : { background: "var(--bg-muted)", color: "var(--text-4)" }}>
            {i + 1}
          </button>
        ))}
      </div>
